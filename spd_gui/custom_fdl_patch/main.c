 ...
 * resposta BSL_REP_READ_FLASH (mesmo truque que read_efuse já usa acima)
 * para não precisar mexer no cliente além de reconhecer esse comando.
 */
static int read_jedec_id(uint8_t *pkt) {
	unsigned len = READ16_BE(pkt + 2);
	uint32_t id;
...
void dl_main(void) {
	...
	for (;;) {
		...
		case MABUIE_CMD_READ_JEDEC_ID:
			ret = read_jedec_id(pkt);
			break;
#endif
